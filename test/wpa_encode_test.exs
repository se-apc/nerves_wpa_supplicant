defmodule WpaEncodeTest do
  ...
  test "interactive" do
    assert Messages.encode({:'CTRL-RSP-IDENTITY', 1, "response text"}) == "CTRL-RSP-IDENTITY-1-response text"
    assert Messages.encode({:'CTRL-RSP-PASSWORD', 1, "response text"}) == "CTRL-RSP-PASSWORD-1-response text"
    assert Messages.encode({:'CTRL-RSP-NEW_PASSWORD', 1, "response text"}) == "CTRL-RSP-NEW_PASSWORD-1-response text"
    assert Messages.encode({:'CTRL-RSP-PIN', 1, "response text"}) == "CTRL-RSP-PIN-1-response text"
    assert Messages.encode({:'CTRL-RSP-OTP', 1, "response text"}) == "CTRL-RSP-OTP-1-response text"
    assert Messages.encode({:'CTRL-RSP-PASSPHRASE', 1, "response text"}) == "CTRL-RSP-PASSPHRASE-1-response text"
  end
  ...
  test "commands" do
    ...
    assert Messages.encode({:SET, :string_variable, "string"}) == "SET string_variable \"string\""
    ...
    assert Messages.encode({:PREAUTH, "00:09:5b:95:e0:4e"}) == "PREAUTH 00:09:5b:95:e0:4e"
    ...
    assert Messages.encode({:BSSID, 1, "00:09:5b:95:e0:4e"}) == "BSSID 1 00:09:5b:95:e0:4e"
    ...
    assert Messages.encode(:P2P_SERV_DISC_CANCEL_REQ) == "P2P_SERV_DISC_CANCEL_REQ"
